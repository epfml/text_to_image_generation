...
def main():
    ...
    len_train = int(abs(len_dataset * 0.8))
    train_dataset, val_dataset = random_split(dataset, [len_train, len_dataset - len_train])
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=1, drop_last=True)
    ...
    data = get_iterator(train_dataloader)
    val_data = get_iterator(val_dataloader)
